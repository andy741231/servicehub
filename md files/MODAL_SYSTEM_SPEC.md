...
## Overview
This document defines the consistent modal system for the application, based on the existing `client/src/components/Dialog.jsx` component and best practices identified across the codebase.

## Core Principles

### 1. Use Shared Components
- **Primary**: `client/src/components/Dialog.jsx` for all confirmation, alert, and prompt dialogs
- **Secondary**: Custom inline modals only when complex content is needed
- **Toast**: `client/src/components/Toast.jsx` for non-blocking notifications
...
### usePrompt() Hook
Replaces `window.prompt()` for text input dialogs.
...
const name = await promptDialog({
  ...
  message: 'Enter a name below.',  // optional
  defaultValue: 'New Folder',     // optional, default ''
  placeholder: 'Type here...',    // optional, default ''
  confirmLabel: 'Create',         // optional, default 'OK'
  cancelLabel: 'Cancel',          // optional, default 'Cancel'
  variant: 'default',             // 'danger' | 'warning' | 'default'
});
...
- [ ] Use `usePrompt()` for text input dialogs
...
- ✅ client/src/pages/web/Pages.jsx - uses `useConfirm()` and `useToast()`
- ✅ client/src/pages/web/Assets.jsx - uses `useConfirm()` and `useToast()`
- ✅ client/src/pages/web/HeaderFooter.jsx - uses `useToast()`
- ✅ client/src/pages/web/OldWebBuilder.jsx - uses `useToast()`
- ✅ client/src/pages/forms/components/PropertiesPanel.jsx - uses `useToast()` (migrated)
- ✅ client/src/pages/forms/components/FormRenderer.jsx - uses `useToast()` and `useAlert()` (migrated)
- ✅ client/src/pages/forms/FormsDashboard.jsx - uses `useConfirm()` (migrated)
- ✅ client/src/pages/forms/FormsList.jsx - uses `useConfirm()` and `useToast()` (migrated)
- ✅ client/src/pages/email/EmailDashboard.jsx - uses `useConfirm()` (migrated)
- ✅ client/src/pages/email/CampaignComposer.jsx - uses `useAlert()` (migrated)

### Custom Modals (Spec-Compliant)
All custom modals have been updated to meet the spec:
- z-index: `z-[9999]`
- Backdrop: `bg-black/40 backdrop-blur-[2px]`
- Container: `rounded-2xl shadow-2xl animate-[fadeInScale_0.15s_ease-out]`
- Accessibility: `role="dialog"`, `aria-modal="true"`, `aria-labelledby`
- Escape key handler
- Click-outside-to-close

- ✅ client/src/pages/admin/Users.jsx - 2 custom modals (compliant)
- ✅ client/src/pages/web/Pages.jsx - 1 custom modal (compliant)
- ✅ client/src/pages/web/InlineEditor.jsx - 8 custom modals (compliant)
- ✅ client/src/pages/forms/FormsList.jsx - 3 custom modals (compliant)
- ✅ client/src/pages/forms/FormsBuilder.jsx - 2 custom modals (compliant)
- ✅ client/src/pages/forms/Submissions.jsx - 1 custom modal (compliant)
- ✅ client/src/pages/email/MailingLists.jsx - 3 custom modals (compliant)
- ✅ client/src/pages/email/CampaignComposer.jsx - 1 custom modal (compliant)

### No Remaining `window.confirm()`, `alert()`, or `window.prompt()` Calls
All native browser dialogs have been migrated to the shared system.
